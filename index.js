...
const client = new octokit.Octokit({ auth: process.env.GITHUB_ACCESS_TOKEN });

async function updateAllRepos() {
	try {
		const response = await client.request("GET /repos/wribeiiro/books", {
			sort: "updated",
			per_page: "100",
		});
		const repos = response.data.filter(r => r.name !== "geraldiner" && r.name !== "projects-readme" && r.name !== "projects-readme-tutorial");
